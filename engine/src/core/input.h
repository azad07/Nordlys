...
/**
 * @brief Mouse button, just covering basic mouse button, 
 * TODO: can be extended to support game mouse with more buttons.
 * 
 */
...
/**
 * @brief  Used to define key code enum.
 * Usage: DEFINE_KEY(NAME, code) 
 * extends to KEY_NAME = code
 */
...
NAPI b8 input_is_key_down(keys key);
NAPI b8 input_is_key_up(keys key);
NAPI b8 input_was_key_down(keys key);
NAPI b8 input_was_key_up(keys key);

void input_process_keyboard_key(keys key, b8 pressed);

/* Mouse input system. */
NAPI b8 input_is_button_down(buttons button);
NAPI b8 input_is_button_up(buttons button);
NAPI b8 input_was_button_down(buttons button);
NAPI b8 input_was_button_up(buttons button);
NAPI void input_get_mouse_position(i32* x, i32* y);
NAPI void input_get_previous_mouse_position(i32* x, i32 *y);
