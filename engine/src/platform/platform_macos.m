...
- (BOOL)windowShouldClose:(id)sender {
  ...
  // FIXME: event_context data = {};
  // FIXME: event_fire(EVENT_CODE_APPLICATION_QUIT, 0, data);

  return YES;
}

- (void)windowDidResize:(NSNotification *)notification {
  // FIXME: event_context context;
  const NSRect contentRect = [state->view frame];
  const NSRect framebufferRect = [state->view convertRectToBacking:contentRect];
  // FIXME: context.data.u16[0] = (u16)framebufferRect.size.width;
  // FIXME: context.data.u16[1] = (u16)framebufferRect.size.height;
  //  FIXME: event_fire(EVENT_CODE_RESIZED, 0, context);
}

- (void)windowDidMiniaturize:(NSNotification *)notification {
  // FIXME: event_context context;
  // FIXME: context.data.u16[0] = 0;
  // FIXME: context.data.u16[1] = 0;
  // FIXME: event_fire(EVENT_CODE_RESIZED, 0, context);
...
b8 platform_initialize(platform_state *plat_state, const char *application_name,
  ...
  @autoreleasepool {
    ...
    if (!state->app_delegate) {
      NERROR("Failed to create application delegate")
      return FALSE;
    }
    ...
    if (!state->wnd_delegate) {
      NERROR("Failed to create window delegate")
      return FALSE;
    }
        ...
        [[NSWindow alloc] initWithContentRect:NSMakeRect(x, y, width, height)
                                    styleMask:NSWindowStyleMaskMiniaturizable |
                                      ...
                                      backing:NSBackingStoreBuffered
                                        defer:NO];
    if (!state->window) {
      NERROR("Failed to create window");
      return FALSE;
    }
    ...
    if (!state->layer) {
      NERROR("Failed to create layer for view");
    }
...
b8 platform_create_vulkan_surface(platform_state *plat_state,
  ...
  if (result != VK_SUCCESS) {
    NFATAL("Vulkan surface creation failed.");
    return FALSE;
  }
